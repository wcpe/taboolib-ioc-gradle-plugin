...
目标：让 consumer 侧静态诊断在默认配置下真正成为可定位、可追踪的构建质量门，只有显式关闭时才退回报告模式。

任务：
- [x] 调整错误样例模块默认行为，移除默认跳过 error/warning 质量门的示例配置，让 `build` 在发现 error 时直接失败。
- [x] 强化 `analyzeTaboolibIocBeans` 失败提示，直接输出规则名、注入点、依赖类型、候选 Bean 与报告路径，避免终端里只看到计数。
- [x] 为静态诊断补充源码绝对路径与行号输出，控制台按 IDE 可识别的问题格式打印，支持点击跳转定位。
- [x] 把静态诊断接入 Gradle Problems API，让 `problems-report.html` 与 IDE 结构化问题视图也能看到 Bean 装配问题。
- [x] 同步 README、CI 与验证用例，明确“默认拦截，显式配置才跳过”的使用方式，并覆盖 `build` 级别拦截验证。

方案：保持 `analysisFailOnError=true` 的默认约定不变，把此前只存在于健康样例的质量门体验下沉到错误样例与真实构建日志中，同时用 CI 显式传参演示报告模式。

完成标准：错误样例模块在未显式关闭质量门时执行 `build` 会被静态诊断拦截，终端可直接看到可定位的错误明细；CI 如需继续生成错误样例报告，必须显式传入跳过参数。
